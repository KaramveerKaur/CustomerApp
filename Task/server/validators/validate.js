...
module.exports = {
    ...
    checkInputDataQuality: function(req, res) {
        ...
        if (req.body.emailId == "") return res.send(resultsNotFound);
        if (req.body.password == "") return res.send(resultsNotFound);
        if (req.body.fullName == "") return res.send(resultsNotFound);
        if (req.body.gender == "") return res.send(resultsNotFound);
        if (req.body.orgName == "") return res.send(resultsNotFound);
      },
